...
/**
 * Base class for all Workflow Processors. Responsible of keeping track of an ordered collection of {@link Activity Activities}
 * 
 */
public abstract class BaseWorkflow implements Workflow, BeanFactoryAware {
	...
	/*
	 * Sets name of the spring bean in the application context that this processor is configured under (non-Javadoc)
	 * 
	...
	/*
	 * Sets the spring bean factroy bean that is responsible for this processor. (non-Javadoc)
	 * 
	...
	public void afterPropertiesSet() throws Exception {
		...
		for (Iterator<Activity> iter = activities.iterator(); iter.hasNext();) {
			...
			if (!supports(activitiy))
				throw new BeanInitializationException("The workflow processor [" + beanName + "] does " + "not support the activity of type"
						+ activitiy.getClass().getName());
		}
	}
	...
	protected String getBeanDesc() {
		return (beanFactory instanceof ConfigurableListableBeanFactory) ? ((ConfigurableListableBeanFactory) beanFactory).getBeanDefinition(beanName)
				.getResourceDescription() : "Workflow Processor: " + beanName;
	}
